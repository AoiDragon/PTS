            ...
            dimension_info = [
                {
                    ...
                    "absolute_size": edge1 + edge2 + edge3
                },
            ]
            existing_boxes.append(bounding_box)
            return dimension_info
            ...
            dimension_info = [
                {
                    ...
                    "absolute_size": edge1 + edge2 + edge3 + edge4
                },
            ]
            existing_boxes.append(bounding_box)
            return dimension_info
...
def generate_images(num_images, output_dir):
    ...
    with open(jsonl_output_path, "w", encoding="utf-8") as f:
        for i in tqdm(range(num_images)):
            ...
            dimension_info_list = [
                {
                    ...
                    "absolute_size": width*2 + height*2
                },
            ]
            ...
            reference_length = min(side1["absolute_size"], side2["absolute_size"])
            question_template = random.choice(question_template_list)
            relative_length1 = round(side1["absolute_size"] / reference_length, 1)
            relative_length2 = round(side2["absolute_size"] / reference_length, 1)

            # shorter_side = side1 if side1["absolute_size"] < side2["absolute_size"] else side2
            # longer_side = side2 if side1["absolute_size"] < side2["absolute_size"] else side1

            # reference_length = shorter_side["absolute_size"]
            # question_template = random.choice(question_template_list)

            eval_item = {
                "id": f"grpo_1d_perimeter_normal_{str(i+1).zfill(6)}",
                "images": [full_image_name],
                "problem": question_template.format(side2["name"], side1["name"]),
                "answer": round(side2["absolute_size"] / side1["absolute_size"], 2)
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), '../..', 'grpo', '1d_perimeter'))
