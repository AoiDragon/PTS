...
def generate_images(num_images, output_dir):
    ...
    with open(jsonl_output_path, "w", encoding="utf-8") as f:
        for i in range(num_images):
            image_name = f"train_1d_length_{str(i+1).zfill(6)}.png"
            ...
            output_json = {
                "id": f"train_1d_length_{str(i+1).zfill(6)}",
                "image": full_image_name,
                "side1": {
                    "name": side1["name"],
                    "relative_length": relative_length1,
                    "symbolization_result": visualize(relative_length1)
                },
                "side2": {
                    "name": side2["name"],
                    "relative_length": relative_length2,
                    "symbolization_result": visualize(relative_length2)
                },
                "question": question_template.format(side2["name"], side1["name"]),
                "answer": round(relative_length2 / relative_length1, 2),
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", type=str, default=os.path.join(os.path.dirname(os.path.abspath(__file__)), '../..', 'annotation', '1d_length'))
